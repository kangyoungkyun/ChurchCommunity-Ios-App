...
import UIKit
import Firebase
class JobAHVC: UITableViewController ,UISearchBarDelegate{
    ...
    override func viewDidLoad() {
        ...
        tableView.register(JobsCell.self, forCellReuseIdentifier: cellId)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath) as? JobsCell
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //선택한 셀 정보 가져오기
        let cell = tableView.cellForRow(at: indexPath) as? JobsCell
        ...
        //여기가 문제
        let ref = Database.database().reference()
        ref.child("jobs").child(pid!).updateChildValues(hiting)
        ...
        //디테일 페이지로 이동
        let detailTalkViewController = JobDetailVC()
    ...
    func showPost(){
        ...
        let ref = Database.database().reference()
        ref.child("jobs").queryOrdered(byChild: "date").observe(.value) { (snapshot) in
