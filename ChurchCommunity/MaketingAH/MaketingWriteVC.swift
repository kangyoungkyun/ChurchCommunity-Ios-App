...
import UIKit
import Firebase
class MaketingWriteVC: UIViewController ,UITextViewDelegate{
    ...
    @objc func writeAction(){
        ...
        if let CurrentUser = Auth.auth().currentUser{
            ...
            //랜덤 키
            let PostKey = ref.child("maketings").childByAutoId().key
            //부모키 user를 만들고 그 밑에 각자의 아이디로 또 자식을 만든다.
            let PostReference = ref.child("maketings").child(PostKey)
