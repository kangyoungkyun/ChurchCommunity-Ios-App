...
import UIKit
import Firebase
class MaketingDetailVC: UIViewController, UITableViewDelegate,UITableViewDataSource {
    ...
    func popUpController(txt:String,rid:String)
    {
        ...
        let somethingAction = UIAlertAction(title: "수정", style: UIAlertActionStyle.default, handler: {(alert: UIAlertAction!) in //print("something")
            ...
            ref.child("maketingReplys").child(self.pidLabel.text!).child(rid).updateChildValues(["text":self.modifyText ?? "",
    ...
    func tableView(_ replyView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let deleteAction = UIAlertAction(title: "댓글삭제", style: .destructive) { (alert) in
            if(Auth.auth().currentUser?.uid == uid){
                let ref = Database.database().reference()
                ref.child("maketingReplys").child(self.pidLabel.text!).child(rid!).removeValue()
        ...
        ref.child("users").child(myUid!).observe(.value) { (snapshot) in
            ...
            if let pass = childValue["pass"] as? String{
                if pass == "n"{
                    ...
                    //랜덤 키
                    let replyRef = ref.child("maketingReplys").child(self.pidLabel.text!)
                    
                    let replyKey = ref.child("maketingReplys").childByAutoId().key
                    print(replyKey)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "홍보정보"
    ...
    func fetchReply(){
        let ref = Database.database().reference()
        ref.child("maketingReplys").child(pidLabel.text!).queryOrdered(byChild: "date").observe(.value) { (snapshot) in
            ...
            if(Int(snapshot.childrenCount) == 0){
                ...
                self.replyCount = 0
                ref.child("maketings").child(self.pidLabel.text!).updateChildValues(["reply": self.replyCount ?? 0])
    ...
    @objc func goSettingAlertAction(){
        ...
        let deleteAction = UIAlertAction(title: "글삭제", style: .destructive) { (alert) in
            ...
            if(replyNum == 0){
                let ref = Database.database().reference()
                ref.child("maketings").child(self.pidLabel.text!).removeValue()
    ...
    func settingAlertAction(txt:String, pid:String)
    {
        ...
        let somethingAction = UIAlertAction(title: "수정", style: UIAlertActionStyle.default, handler: {(alert: UIAlertAction!) in //print("something")
            ...
            let ref = Database.database().reference()
            ref.child("maketings").child(pid).updateChildValues(["text":self.modifyText ?? "",
                                                            "date":ServerValue.timestamp()])
