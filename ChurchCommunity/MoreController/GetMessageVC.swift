...
import UIKit
import Firebase
class GetMessageVC: UITableViewController {
    var msgs = [Message]()
    var cellId = "getMsg"
    var myUid : String?
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "받은쪽지"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.nameLabel.text = "From.\(msgs[indexPath.row].name!)"
    ...
    func showMessage(){
        ...
        ref.child("messages").queryOrdered(byChild: "date").observe(.value) { (snapshot) in
            ...
            for child in snapshot.children{
                ...
                if let name = childValue["fromname"], let date = childValue["date"], let toid = childValue["toid"], let text = childValue["content"]{
                    
                    if(toid as? String == myUid!){
